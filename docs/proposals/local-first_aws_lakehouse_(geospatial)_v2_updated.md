...
- **Orchestration**: **Apache Airflow** (locally in Docker, and AWS-managed in prod) triggers Glue jobs and dbt runs.
...
- **Orchestration**: Airflow container (webserver, scheduler, Postgres metadata DB)
- **Transformations (Silver→Gold)**: dbt Core project running via Airflow (dbt CLI) against Trino
...
- **SQL**: Athena
- **Orchestration**: Apache Airflow (e.g. MWAA or self-managed in ECS/EKS) in VPC
- **Transformations (Silver→Gold)**: dbt Core running in Airflow against Athena (dbt-athena adapter)
```mermaid
flowchart LR
...
- If validation fails: write a structured error report to `tm-lakehouse-quarantine/...` and **raise** so the Airflow task / DAG run fails visibly.

## Logging and error-handling standards

- Use Python `logging` (or `structlog`) with structured metadata from within your Spark jobs and dbt/SQL code.
...
- AWS execution context:
  - Glue jobs and Airflow use their own job parameters/roles via IAM.
...
- `just ui` (prints URLs for Airflow, Trino, MinIO)
- `just bronze-upload` (Airflow DAG or local Spark job)
- `just silver` (Airflow DAG to run Spark Silver job)
- `just gold` (Airflow DAG to run dbt models)
- `just compact`
...
- Add validation tasks (Airflow + Spark/dbt) with explicit thresholds.
- Emit quarantine objects + raise errors so Airflow tasks fail visibly.
...
- Airflow in VPC triggers Glue jobs and dbt runs
...
- Airflow DAGs: e.g. `orchestration/airflow/dags/bronze_ingest_dag.py`, `orchestration/airflow/dags/silver_etl_dag.py`, `orchestration/airflow/dags/gold_dbt_dag.py`
- dbt project: `transform/dbt_project/` (models for Silver and Gold, `dbt_project.yml`, profiles)
